...
- Locate the approved system using the user's path or repository conventions,
  then `docs/design-system.html`, `DESIGN.md`, token/theme files, CSS variables,
  and installed component examples. Existing native systems can be valid inputs;
  an HTML system page is not universally required. File existence alone does not
...
|---|---|
| Existing React project needs a design foundation | [init-design-system](../init-design-system/SKILL.md): `/uikit:init-design-system`. Explain that it creates a visual system page and wires real components into the app. |
...
Treat these as entry points, not a sequence everyone must repeat. A Vue project
with an approved system can go to prototype or implement. Do not recommend React
initialization or a migration just to use UIkit. For a non-React project without a
system, explain the initialization skill's scope and recommend establishing an
approved native system first. Do not invent a UIkit command for that missing step.
...
- **Init:** Needs a React project for its default shadcn/ui setup. Existing tokens
  are reused; this command changes the app as well as producing documentation.
